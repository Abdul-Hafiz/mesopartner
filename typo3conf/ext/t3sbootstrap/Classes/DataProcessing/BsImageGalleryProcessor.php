...
class BsImageGalleryProcessor implements DataProcessorInterface
{
	...
	public function process(ContentObjectRenderer $cObj, array $contentObjectConfiguration, array $processorConfiguration,	array $processedData)
	{
		...
		if (!empty($processorConfiguration['references.'])) {
			...
			if (!empty($relationField)) {
				...
				if (!empty($relationTable)) {
					...
					if(empty($fileCollector->getFiles())) {
						// fallback to field media - before 3.2.4
						$fileCollector->addFilesFromRelation($relationTable, 'media', $cObj->data);
					}
				}
			}
		}
		...
		$numberOfColumns = (int)$processedData['data']['imagecols'];
		$galleryChunk = '';
